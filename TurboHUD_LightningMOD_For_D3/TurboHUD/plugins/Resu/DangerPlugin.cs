...
namespace Turbo.Plugins.Resu
{
...

    {
        ...
        public bool ShowWarning { get; set; } // @modified
        ...
        public void PaintWorld(WorldLayer layer)
        {
            ...
            foreach (var actor in danger)
            {
                ...
                if (!Hud.Game.Me.IsDead)
                {
                    ...
                    {
                        // @modified
                        if(ShowWarning) MoveWarningDecorator.Paint(layer, actor, actor.FloorCoordinate, "Двигайся!");
                        Danger = false;
                    }
                    else Danger = true;
                }
                ...
                // @modified удалено переключение, теперь EliteMonsterSkillPlugin работает все время
            ...
            // @modified
            if (ShowWarning && RunForYourLife && Danger && !Hud.Game.Me.IsDead) MoveWarningDecorator.Paint(layer, null, Hud.Game.Me.FloorCoordinate, "Опасность! " + (int)Hud.Game.Me.Defense.HealthPct + "%");

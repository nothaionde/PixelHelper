...
{
    public class BarbarianWhirlwindPlugin1 : BasePlugin, IInGameTopPainter, IKeyEventHandler
    {
        ...
        public BarbarianWhirlwindPlugin1()
        {
            Enabled = false;
            isHideTip = false;
        }
        ...
        {
            if (Hud.GetPlugin<PickUpPlugin1>().WaitingforClick == false && ToggleKeyEvent.Matches(keyEvent) && keyEvent.IsPressed && isWhirlwindBarbarian && !Hud.Inventory.InventoryMainUiElement.Visible)
            {
                if(Running)
                {

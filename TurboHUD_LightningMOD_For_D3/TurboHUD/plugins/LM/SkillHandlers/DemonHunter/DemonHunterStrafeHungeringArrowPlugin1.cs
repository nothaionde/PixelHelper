namespace Turbo.Plugins.LM
{
    ...
    public class DemonHunterStrafeHungeringArrowPlugin1 : BasePlugin, IInGameTopPainter, IKeyEventHandler
    {
        ...
        public DemonHunterStrafeHungeringArrowPlugin1()
        {
            Enabled = false;
            dongneng = 21;
            isHideTip = false;
        ...
        {
            if (Hud.GetPlugin<PickUpPlugin1>().WaitingforClick == false && ToggleKeyEvent.Matches(keyEvent) && keyEvent.IsPressed && isStrafeDH && !Hud.Inventory.InventoryMainUiElement.Visible)
            {
                if(Running)
                {
                    Running = false;
        ...
        public void PaintTopInGame(ClipState clipState)
        {
            ...
            if (Hud.Inventory.InventoryMainUiElement.Visible || Hud.GetPlugin<PickUpPlugin1>().WaitingforClick == true)
            {
                if (Running)
                {
                    upStrafe();
            ...
            if (Running)
            {
                ...
                || Hud.GetPlugin<PickUpPlugin1>().isItemsAround == true
                || Hud.GetPlugin<OpenChestPlugin1>().Clicking == true
